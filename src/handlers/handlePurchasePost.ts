...
export const handlePurhcasePost = async (userId: string,postId: string) => {
    ...
    try {
        ...
        const wallet = await CreditWallet.findOne({ user: userId }).session(session);
        if (!wallet) {
        ...
        await CreditWallet.findOneAndUpdate({ user: userId }, { $inc: { credit: -10 } }).session(session);

        await WalletServices.updateBalanceOfCreator(post.user as any,10,session)
